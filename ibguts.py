...
import argparse, ast, os, platform, subprocess, tempfile, textwrap, glob
...
class Planner(object):
  ...
  def GetCcArgs(self):
    return [ self.cfg.cc.tool, '-I' + self.src_root, '-I' + self.out_root ]  \
        ...
        + [ '-DIB_SRC_ROOT=' + self.src_root.replace('\\', '\\\\'),
            '-DIB_OUT_ROOT=' + self.out_root.replace('\\', '\\\\') ]  \
        + self.cfg.cc.flags
...
class IbRunner(object):
  ...
  def GetCmdArgs():
    ...
    parser.add_argument(
        ...
        help="The root of output tree. If relative, this path is relative to "
             "the root of the source tree. The default is '.'.")
    parser.add_argument(
        '--cfg', default=cfg,
  ...
  def ProcessRawInput(args):
    ...
    elif not args.cfg_root and os.path.dirname(args.cfg) != args.src_root:
      args.cfg_root = os.path.dirname(args.cfg)
    else:
      args.cfg_root = args.src_root
